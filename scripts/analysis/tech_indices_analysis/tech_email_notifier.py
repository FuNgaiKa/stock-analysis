...
class TechEmailNotifier:
    ...
    def send_tech_report(self, report: Dict, text_content: str) -> bool:
        """
        发送四大科技指数分析报告邮件
        ...
        # 计算整体趋势
        indices_data = report.get('indices', {})
        bullish_count = 0
        bearish_count = 0

        for index_data in indices_data.values():
            judgment = index_data.get('comprehensive_judgment', {})
        ...
        # 确定邮件主题
        if bullish_count >= 3:
            subject = f"📈 【偏多】四大科技指数分析 - {bullish_count}个看多"
        elif bearish_count >= 3:
            subject = f"📉 【偏空】四大科技指数分析 - {bearish_count}个看空"
        else:
            subject = f"➡️ 【中性】四大科技指数分析 - 多空分化"
...
<div class="container">
    <div class="header">
        <h1>{trend_icon} 四大科技指数综合分析</h1>
        <div class="subtitle">创业板指 | 科创50 | 恒生科技 | 纳斯达克</div>
        <div class="subtitle">整体趋势: {trend_text} | {date}</div>
    </div>
"""

        # 遍历四大指数
        index_order = ['CYBZ', 'KECHUANG50', 'HKTECH', 'NASDAQ']
        for index_key in index_order:
            if index_key not in indices_data:
                continue

            data = indices_data[index_key]
            if 'error' in data:
                continue
            ...
            html += f"""
    <div class="section">
        <div class="section-title">{index_name}</div>

        <div class="index-card">
            <div class="index-header">
                <div class="index-name">当前点位</div>
                <div class="index-price {price_color}">
...
"""
        ...
        html += f"""
    <div class="footer">
        <p>📊 Claude Code 量化分析系统 | 四大科技指数专项分析</p>
