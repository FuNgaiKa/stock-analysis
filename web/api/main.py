...
from strategies.position.market_analyzers.us_market_analyzer import (
    USMarketAnalyzer,
    US_INDICES,
    DEFAULT_US_INDICES
)
...
async def get_hk_indices():
    """获取支持的港股指数列表"""
    try:
        from strategies.position.market_analyzers.hk_market_analyzer import HK_INDICES
        return [
            {
                "code": code,
                "name": config.name,
...
async def get_hk_current_positions(
    ...
    """获取港股指数当前点位"""
    try:
        from strategies.position.market_analyzers.hk_market_analyzer import HKMarketAnalyzer, DEFAULT_HK_INDICES
...
async def get_cn_indices():
    """获取支持的A股指数列表"""
    try:
        from strategies.position.market_analyzers.cn_market_analyzer import CN_INDICES
        return [
            {
                "code": code,
                "name": config.name,
...
async def get_cn_current_positions(
    ...
    """获取A股指数当前点位"""
    try:
        from strategies.position.market_analyzers.cn_market_analyzer import CNMarketAnalyzer, DEFAULT_CN_INDICES
...
async def analyze_correlation(
    ...
    try:
        ...
        from strategies.position.analyzers.technical_analysis.correlation_analyzer import CorrelationAnalyzer
...
async def run_sr_breakout_backtest(
    ...
    try:
        import yfinance as yf
        from strategies.trading.signal_generators.sr_breakout_strategy import SRBreakoutStrategy
        from strategies.trading.backtesting.backtest_engine import BacktestEngine
        from strategies.trading.backtesting.performance_metrics import PerformanceMetrics
