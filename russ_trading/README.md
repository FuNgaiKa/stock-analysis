...
**新用户？** 查看以下文档：
- 📖 **[快速使用指南 (QUICK_START.md)](docs/QUICK_START.md)** - 了解如何生成每日报告
- 📚 **[文档中心 (docs/)](docs/)** - 查看所有相关文档索引
...
**对Claude说**：`帮我生成今天的市场洞察报告`

**说明**: 从2025-11-18起,持仓调整建议已整合到市场洞察报告中,只需运行一个命令即可
...
# 分析指定资产
python -m russ_trading.runners.run_unified_analysis --assets CYBZ HKTECH
```
...
30 17 * * 1-5 cd /Users/russ/PycharmProjects/stock-analysis && \
    /usr/local/bin/python3 -m russ_trading.runners.run_unified_analysis --email >> logs/market_insight.log 2>&1
```
...
```bash
# 生成当月计划
python -m russ_trading.generators.monthly_plan_generator --month 2025-11

# 指定市场数据
python -m russ_trading.generators.monthly_plan_generator \
    --month 2025-11 \
    --market-data data/market_data.json
```
...
└── daily/
    └── YYYY-MM/
        └── 市场洞察报告_YYYYMMDD.md     ← 每日自动生成 + 邮件(已整合持仓调整建议)
```
...
### 每日流程(自动化)
1. **早盘前**: 查看昨日自动生成的报告
   - 📄 `reports/daily/2025-11/市场洞察报告_昨日日期.md`
2. **盘中**: 观察持仓走势,记录重要信号
3. **下午17:30**: 系统自动生成并发送今日市场洞察报告(邮件)
4. **晚上18:00**:
   - 查看邮件中的市场洞察报告
   - 查看持仓分析和调整建议
   - 查看看多/看空标的筛选
...
- 🔄 [迁移计划](docs/MIGRATION_PLAN.md) - 重构详细计划
- ✅ [重构总结](docs/REFACTORING_SUMMARY.md) - 重构成果
